...
function M.build_spec(opts)
  ...
  if ft == "python" then
    return { cmd = { "python3", "-m", "compileall", dir }, cwd = dir, artifact = nil, title = "python -m compileall" }
  end
  ...
  if ft == "c" or ft == "cpp" then
    if is_cmake_project(root) then
      ...
      if not is_dir(bdir) then
        return {
          cmd = { "sh", "-lc", string.format("cd %q && cmake -S . -B %q -DCMAKE_BUILD_TYPE=Release && cmake --build %q", root, bdir, bdir) },
          cwd = root,
          artifact = nil,
...
function M.clean_spec()
  ...
  if ft == "markdown" then
    ...
    return { cmd = { "sh", "-lc", "rm -f " .. vim.fn.shellescape(pdf) .. " " .. vim.fn.shellescape(html) }, cwd = dir, title = "rm outputs" }
  end
